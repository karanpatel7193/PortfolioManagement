...
export class MultiSelectComponent implements ControlValueAccessor {
    ...
    public oldSelectedItems: Array<ListItem> = [];
    ...
    defaultSettings: IDropdownSettings = {
        ...
        noFilteredDataAvailablePlaceholderText: "No result found",
        closeDropDownOnSelection: false,
        showSelectedItemsAtTop: false,
        defaultOpen: false,
        allowRemoteDataSearch: false,
        showOnlySelectedCount: false
    };
    ...
    onItemClick($event: any, item: ListItem): any {
        ...
        if (this._settings) {
            ...
            const allowAdd = this._settings.limitSelection === -1 || (this._settings.limitSelection && this._settings.limitSelection > 0 && this.selectedItems.length < this._settings.limitSelection);
            if (!found) {
                if (allowAdd) {
    ...
    writeValue(value: any) {
        if (value !== undefined && value !== null && value.length > 0) {
            if (this._settings && this._settings.singleSelection) {
                try {
                    if (value.length >= 1) {
                ...
                if (this._settings && this._settings.limitSelection && this._settings.limitSelection > 0) {
    ...
    itemShowRemaining(): number {
        if (this._settings && this._settings.itemsShowLimit)
    ...
    objectify(val: ListItem) {
        if (this._sourceDataType === 'object' && this._settings && this._settings.idField && this._settings.textField) {
            const obj: any = {};
            obj[this._settings.idField] = val.id;
            obj[this._settings.textField] = val.text;
            if (this._settings.disabledField && this._sourceDataFields.includes(this._settings.disabledField)) {
                obj[this._settings.disabledField] = val.isDisabled;
            }
            if (this._settings.tooltipField && this._sourceDataFields.includes(this._settings.tooltipField)) {
    ...
    private deobjectify(item: any): any {
        if (typeof item === "string" || typeof item === "number") {
            return new ListItem(item);
        } else {
            if(this._settings.idField && this._settings.textField && this._settings.tooltipField && this._settings.disabledField)
                return new ListItem({
    ...
    private isChangeOldNew(): any {
        let isChange: Boolean = false;
        for(let i = 0; i < this.selectedItems.length; i++) {
            let isMatch : boolean = false;
            for (let j = 0; j < this.oldSelectedItems.length; j++) {
                if(this.selectedItems[i].id == this.oldSelectedItems[i].id){
                    isMatch = true;
                    break;
                }
            }
